...
* Once the device has been paired, or if it is already paired, HomeSpan waits to receive its first inbound secure connection request from HomeKit (usually from one of your HomeKit Hubs) and sets its state to **HS_CONNECTED** upon receiving and verifying this connection.

* Whenever the HomeSpan state is set to **HS_CONNECTED**, the device will respond to Accessory control requests from the Home App or by Siri.  With the one exception below, the device will not response to Accessory control requests from the Home App or Siri if HomeSpan is in any other state.[^weblog]
